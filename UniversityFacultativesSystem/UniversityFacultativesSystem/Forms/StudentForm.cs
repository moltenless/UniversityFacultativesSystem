...
namespace UniversityFacultativesSystem
{
    internal partial class StudentForm : Form
    {
        ...
        public StudentForm(string user, string password)
        {
            ...
            var logins = newService.Logins.GetStudentsLogins();
            newService.Close();
            ...
            label3.Text = current.Group;
            label16.Text = current.Address;
            label18.Text = current.Phone;
        }
        ...
        {
            current = service.Students.Get(current.Id);
            label20.Text = current.CoursesTaken.ToString();
            ...
            string[] titles = { "id", "Last Name", "First Name", "Middle Name", "Department" };
            for (int i = 0; i < 5; i++)
            {
...

            {
                ...
                titles = new string[] { "id", "student id", "course id" };
                for (int i = 0; i < 3; i++)
                {
                    Label label = new Label();
                    label.TextAlign = ContentAlignment.MiddleCenter;
        ...
        {
            service.Enrollments.AddNew(new Enrollment(-1, current.Id, int.Parse(courseid.Text)));
